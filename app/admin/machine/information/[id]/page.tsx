...
export default function MachineInformation() {
  ...
  useEffect(() => {
    // ดึงข้อมูลเครื่องจาก localStorage
    ...
    if (currentProfile) {
      ...
      if (savedTransactions) {
        ...
        const machine = transactions.find((t: any) => t.id === machineId)
        if (machine) {
          // อัพเดทข้อมูลเครื่องจาก transaction ที่เลือก
          setMachineInfo({
            id: machine.id,
            name: machine.id, // ใช้ ID เป็นชื่อ
