...
export default function EventEdit() {
  ...
  const handleSubmit = () => {
    try {
      // If type changed to Department
      if (formData.type === 'Department') {
        // 1. Get existing event data
        const existingEvent = eventAPI.getTransactionById(eventId);
        if (!existingEvent) {
          console.error('Event not found');
          return;
        }
  
        // 2. Add as new department
        departmentAPI.addTransaction({
          ...formData,
          frames: selectedFrames.map(frame => frame.no),
          stickers: selectedStickers.map(sticker => sticker.no),
          totalSale: existingEvent.totalSale || 0,
        ...
        eventAPI.deleteTransaction(eventId);
        
        // 4. Redirect to department page
        router.push('/admin/dashboard/department');
      } else {
        // If staying as Event, just update
        eventAPI.updateTransaction(eventId, {
          ...formData,
          frames: selectedFrames.map(frame => frame.no),
          stickers: selectedStickers.map(sticker => sticker.no)
        });
        router.push(`/admin/dashboard/event/information/${eventId}`);
      }
    } catch (error) {
          ...
          <div className="min-h-screen bg-[#F7F7F7]">
            ...
            <div className="h-[4rem] bg-white flex justify-between items-center px-6 shadow-sm">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[#61616A]">Dashboard event</span>
